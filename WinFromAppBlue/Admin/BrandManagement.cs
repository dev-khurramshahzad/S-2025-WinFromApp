...
namespace WinFromAppBlue.Admin
{
    public partial class BrandManagement : Form
    {
        ...
        {
            SqlCommand cmd = new SqlCommand($"INSERT INTO Brands VALUES ('{txtName.Text}','{txtDetail.Text}','Image Path','{ddlStatus.SelectedItem.ToString()}')", con);
            con.Open();
            cmd.ExecuteNonQuery();
            con.Close();
        ...
        private void btnDelete_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                SqlCommand cmd = new SqlCommand($"SELECT * FROM Brands Where BrandID = '{txtID.Text}'", con);
                SqlDataAdapter da = new SqlDataAdapter(cmd);
                DataTable dt = new DataTable();
                da.Fill(dt);
                ...
                {
                    cmd = new SqlCommand($"DELETE FROM Brands Where BrandID = '{txtID.Text}'", con);
                    con.Open();
                    cmd.ExecuteNonQuery();
                    con.Close();
        ...
        private void btnUpdate_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (respose == DialogResult.Yes)
                {
                    SqlCommand cmd = new SqlCommand($"UPDATE Brands SET BrandName = '{txtName.Text}', Details = '{txtDetail.Text}' Where BrandID = '{txtID.Text}'", con);
                    con.Open();
                    cmd.ExecuteNonQuery();
                    con.Close();
